...
/**
 * Rotates the WP password of users on WordPress.com sites.
 */
...
final class WPCOM_Site_WP_User_Password_Rotate extends Command {
	...
	protected function configure(): void {
		$this->setDescription( 'Rotates the WordPress user password of a given user on WordPress.com sites.' )
			->setHelp( 'This command allows you to rotate the WP password of users on WordPress.com sites. Finally, it attempts to update the 1Password values of rotated passwords as well.' );

		$this->addArgument( 'site', InputArgument::OPTIONAL, 'The domain or numeric WordPress.com ID of the site on which to rotate the WP user password.' )
	...
	protected function initialize( InputInterface $input, OutputInterface $output ): void {
		...
		$this->sites = match ( $this->multiple ) {
			'all' => get_wpcom_sites(),
			'related' => \array_merge( ...get_wpcom_related_sites( $site->ID ) ),
			default => array( $site ),
		};
	}
	...
	protected function interact( InputInterface $input, OutputInterface $output ): void {
		switch ( $this->multiple ) {
			case 'all':
				...
				break;
			case 'related':
				output_wpcom_related_sites( $output, get_wpcom_related_sites( $this->sites[0]->id ) );
	...
	private function prompt_user_input( InputInterface $input, OutputInterface $output ): ?string {
		...
		if ( 'all' !== $this->multiple ) {
			$site = get_wpcom_sites( $input->getArgument( 'site' ) );
			if ( ! \is_null( $site ) ) {
	 ...
	 * @param   object          $site     The WordPress.com site object.
